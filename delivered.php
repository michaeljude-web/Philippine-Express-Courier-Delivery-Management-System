...
$sql = "SELECT * FROM parcels WHERE (reference_number LIKE ? OR sender_name LIKE ? OR recipient_name LIKE ?) AND status = 'Delivered' LIMIT ?, ?";
$stmt = $conn->prepare($sql);
$searchTermLike = "%" . $searchTerm . "%";
...
$countSql = "SELECT COUNT(*) FROM parcels WHERE (reference_number LIKE ? OR sender_name LIKE ? OR recipient_name LIKE ?) AND status = 'Delivered'";
