<?php session_start(); // Start the session at the top of your PHP file

include "db_connection.php";

$searchTerm = isset($_GET['search']) ? $_GET['search'] : '';

// Your existing SQL query code
$sql = "SELECT * FROM parcels WHERE (reference_number LIKE ? OR sender_name LIKE ? OR recipient_name LIKE ?) AND status = 'Item Accepted by Courier'";
...
// In the section where you update the parcel status
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_status'])) {
    ...
    $staff_id = $_SESSION['user']['id']; // Get staff ID from session
    ...
    try {
        ...
        $insert_sql = "INSERT INTO parcel_status_history (parcel_id, status, staff_id, changed_at) VALUES (?, ?, ?, NOW())";
        $stmt = $conn->prepare($insert_sql);
        if (!$stmt) {
            throw new Exception("Error preparing insert statement: " . $conn->error);
        }
        $stmt->bind_param("isi", $parcel_id, $new_status, $staff_id);
        if (!$stmt->execute()) {
        ...
        $conn->rollback();
        echo "<script>alert('Error updating parcel status: " . $e->getMessage() . "');</script>";
    }
}
...
$sql = "SELECT * FROM parcels WHERE (reference_number LIKE ? OR sender_name LIKE ? OR recipient_name LIKE ?) AND status = 'Item Accepted by Courier' LIMIT ?, ?";
$stmt = $conn->prepare($sql);
$searchTermLike = "%" . $searchTerm . "%";  
...
$countSql = "SELECT COUNT(*) FROM parcels WHERE (reference_number LIKE ? OR sender_name LIKE ? OR recipient_name LIKE ?) AND status = 'Item Accepted by Courier'";
