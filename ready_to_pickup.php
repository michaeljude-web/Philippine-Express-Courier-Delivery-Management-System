...
$sql = "SELECT * FROM parcels WHERE (reference_number LIKE ? OR sender_name LIKE ? OR recipient_name LIKE ?) AND status = 'Ready to Picked Up' LIMIT ?, ?";
$stmt = $conn->prepare($sql);
$searchTermLike = "%" . $searchTerm . "%";
...
$countSql = "SELECT COUNT(*) FROM parcels WHERE (reference_number LIKE ? OR sender_name LIKE ? OR recipient_name LIKE ?) AND status = 'Ready to Picked Up'";
